...
public class KeyExtraction {
    ...
    public Set<String> getKeys(String url, int depth) {
        ...
        if (domain != null) {
            log.info("[ACS-getKeys] try to extracting key for {} ", url);
            try {
    ...
    private static ArrayList<Element> getPotentialKeyPlaces(Document document) {
        ...
        //Scenario 9: Add elements with a specific attribute name that contains "api-key" or "api_key" in a case-insensitive manner.
        apiKeys.addAll(document.select("[i^=api-key], [i^=api_key]"));

        //Scenario 10: Add elements with a specific attribute value that matches any variations of "api-key" or "api_key" in a case-insensitive manner.
        apiKeys.addAll(document.select("[data-custom-attribute~i=^api-key|^api_key]"));

        //Scenario 11: Add elements inside a specific parent element with an ID containing "api-key" or "api_key".
        apiKeys.addAll(document.select("#parent-id [id~=.*\\bapi-key\\b.*], #parent-id [id~=.*\\bapi_key\\b.*]"));
